    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            while (i < Quiz.totalQ) {
                ...
                if (exist == false) {
                    //if not exist push it into questions array
                    Quiz.questionArray.push(new Question(rnd, data.Q[rnd].question, data.Q[rnd].ans1, data.Q[rnd].ans2, data.Q[rnd].ans3, data.Q[rnd].ans4, data.Q[rnd].rightAns));
                    i++;
                }
            }
            //$("#bResult").hide();
...
Quiz.questionToScreen = function () {
    ...
    if (Quiz.nextQuestion < Quiz.totalQ) {
        //if there is next question then show it
        ...
        Quiz.startTimer(); //start new timer
            ...
            //if there isn't next question then calculate result
            Quiz.stopTimer();
            var userScore = 0; // value to hold result
            Quiz.userAnswersArray.forEach(function (item, index) {
                // iterate Quiz.userAnswersArray and if grade user answers
                console.log("index: " + index);
                if (item == Quiz.questionArray[index].rightAns) {
...
Quiz.validateUserAnswer = function () {
    ...
    if ((userAns == "" || userAns == undefined) && Quiz.nextQuestion != 0) {
        //validate that user checked something.. if not popup msg
        return false;
    } else {
        //if user checked something
        return userAns;
    }
...
Quiz.startTimer = function () {
    Quiz.timer = setTimeout(function () {
        if (Quiz.counter < Quiz.timeToAnswer) {
            //if counter small then time to answer that set
...
//# sourceMappingURL=jsQuiz-compiled.js.map